...
```{r}
hist(stepsPerDay, main="Histogram of number of steps per day", 
...
Make a plot:
```{r}
plot(averageNumberOfSteps, type="l", xlab="5 minute interval",
...
```{r}
restoredData <- data
restoredData[rowSums(is.na(data)) > 0, "steps"] <- 
    averageNumberOfSteps[as.character(restoredData[rowSums(is.na(restoredData)) > 0, "interval"])]
```
...
numberOfSteps <- aggregate(steps ~ interval + weekend, data = restoredData, FUN= "mean")
numberOfSteps$weekend <- factor(numberOfSteps$weekend, labels=c("weekday", "weekend"))
library(lattice)
xyplot(steps ~ interval | weekend, data=numberOfSteps, type="l", layout=c(1, 2),
